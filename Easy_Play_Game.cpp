...
void PlayGame2:: swap ( piece p1 , piece p2 ){
...
//
//}
void PlayGame2:: ClickValidate ( sf::RenderWindow &window ){
...
//
//}
void PlayGame2::SCOREINCREMENT(){

    ScoreINT=stoi(ScoreCount);
    ScoreINT+=50;
    ScoreCount= std::to_string(ScoreINT);
}
void PlayGame2::Score(sf::RenderWindow &window){
//    sf::Text SCORE;
    SCORE.setFont(font);
...
//
//}
PlayGame2::PlayGameResult PlayGame2::draw(sf::RenderWindow& window){
    while(true){
        srand(time(0));
//        score();
   ...
   if (!isMoving)
    {
      for(int i=6;i>0;i--)
       for(int j=1;j<=6;j++)
         if (sgrid[i][j].match)
         for(int n=i;n>0;n--)
            if (!sgrid[n][j].match) {swap(sgrid[n][j],
                                          sgrid[i][j]); break;};
      for(int j=1;j<=6;j++)
